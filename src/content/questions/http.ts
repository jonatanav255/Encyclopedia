...
export const bank: QuestionBank = {
  ...
  questions: [
    ...
    {
      level: 'mid',
      question: 'What\'s the difference between `Cache-Control: no-cache`, `no-store`, and `must-revalidate`?',
      answer:
        'They sound similar and aren\'t. Read carefully:\n\n- **`no-store`** — don\'t write to any cache at all. The response disappears after delivery. Use for genuinely sensitive content the user shouldn\'t have on disk.\n- **`no-cache`** — counter-intuitively, this **does allow caching**. It just forbids serving the cached copy without first revalidating with the origin. The cache is used as a validator store, not an answer store. Every request becomes a conditional `If-None-Match`/`If-Modified-Since` round-trip; the origin returns `304` (no body) when content hasn\'t changed.\n- **`must-revalidate`** — only kicks in *when the entry is stale*. While fresh, it serves from cache normally. The difference vs default behavior is that some caches will serve stale content if the origin is unreachable; `must-revalidate` forbids that. Combine with `max-age=N` to get "fresh for N seconds, then strict revalidation."\n\nFor "never cache, ever," **`no-store` alone is enough**. The cargo-culted `no-cache, no-store, must-revalidate, max-age=0` is overkill — the latter three are no-ops once `no-store` is set.\n\nFor "cache, but always check before serving" (auth-aware shells, dashboards that need fresh data but benefit from `304` bandwidth savings), use `no-cache` with an `ETag` so the revalidation is cheap.',
    },
    {
      level: 'senior',
      question: 'What\'s the right `Cache-Control` setup for a CDN-fronted SPA with a JSON API?',
      answer:
        'Different headers for different asset classes — that\'s the whole game.\n\n**Static assets** (fingerprinted JS/CSS/images, e.g. `app.a3f2c.js`):\n\n```\nCache-Control: public, max-age=31536000, immutable\n```\n\nA year, anywhere, never revalidate. The filename will change before the content does. The `immutable` directive tells browsers not to revalidate even on user-triggered reload.\n\n**HTML entrypoint** (`index.html` — needs to update to reference new fingerprinted assets):\n\n```\nCache-Control: public, max-age=0, s-maxage=60,\n               stale-while-revalidate=600\nETag: "..."\n```\n\nBrowser always revalidates (`max-age=0`) but the revalidation is a cheap `304`. The CDN caches for 60 seconds and serves stale-while-revalidate for 10 more minutes, absorbing traffic spikes during deploys.\n\n**JSON API responses** (idempotent, public):\n\n```\nCache-Control: public, max-age=10, stale-while-revalidate=60,\n               stale-if-error=86400\nETag: "..."\n```\n\nFresh for 10s. For the next minute, serve stale immediately while refreshing in the background. If the origin is down, keep serving stale for 24h — degraded but available.\n\n**Per-user JSON** (anything depending on `Authorization`):\n\n```\nCache-Control: private, max-age=10\n```\n\nUser\'s browser may cache; the CDN must not. Forgetting `private` here is **the** classic auth catastrophe — the CDN caches one user\'s response and serves it to everyone.\n\n**Anything secret or audit-sensitive**:\n\n```\nCache-Control: no-store\n```\n\nWith `Vary: Accept-Encoding, Accept-Language` on responses that depend on those headers. Never `Vary: User-Agent` — it shatters the cache into millions of partitions.',
    },
  ],
};
