...
export const bank: QuestionBank = {
  ...
  questions: [
    ...
    {
      level: 'mid',
      question: 'What does a Bloom filter answer, and what does it not?',
      answer:
        'A Bloom filter answers **"have I possibly seen X before?"** with two possible responses:\n\n- **"Definitely not"** — high confidence. The filter has no false negatives.\n- **"Probably yes"** — controllable false-positive rate (typically 1% at ~9.6 bits/element, 0.1% at ~14.4 bits/element).\n\nWhat it does NOT answer:\n- "Exactly how many times have I seen X?" — that\'s count-min sketch.\n- "How many unique items have I seen?" — that\'s HyperLogLog.\n- "Give me back the item." — Bloom only stores membership, not the items themselves.\n\nProduction uses: LSM-tree storage engines (RocksDB, Cassandra, HBase) put a Bloom filter in front of each SSTable so reads can skip SSTables that don\'t contain the key. Bitcoin SPV nodes use them to ask peers "do you have any transactions involving these addresses?" without revealing exact addresses. CDN caches use them to avoid round-trips to origin for known-missing keys.\n\nClassic mistake: trying to delete from a standard Bloom (you can\'t — that\'s what counting Bloom or cuckoo filter exists for).',
    },
    {
      level: 'senior',
      question: 'Why does response time explode as utilization approaches 100%? Cite the M/M/1 formula.',
      answer:
        'For an M/M/1 queue (Poisson arrivals, exponential service, one server), mean response time is:\n\n```\nT = 1 / (μ - λ) = (1 / μ) × 1 / (1 - ρ)\n```\n\nwhere ρ = λ/μ is utilization. The 1/(1-ρ) term is the multiplier vs raw service time:\n\n- ρ = 50%: 2×\n- ρ = 80%: 5×\n- ρ = 90%: 10×\n- ρ = 95%: 20×\n- ρ = 99%: 100×\n\nThis is the **utilization wall**. p99 is even worse — variance compounds, so p99/mean grows faster as ρ → 1.\n\nWhy: arrivals are random. When utilization is high, occasional bursts arrive faster than the server can drain them, and the queue builds. The closer to capacity, the longer it takes the queue to drain. At ρ = 1, the queue grows unboundedly.\n\nPractical implication: target 60-70% steady-state utilization, not 90%. The "headroom" isn\'t waste — it\'s the budget that bounds your tail latency. Cross-reference Little\'s Law (L = λW) for the related capacity-planning identity.',
    },
    {
      level: 'senior',
      question: 'What does Kafka\'s KRaft change, and why was it needed?',
      answer:
        'KRaft (Kafka Raft, KIP-500) replaces ZooKeeper with a Raft-replicated metadata log embedded inside Kafka itself. GA in Kafka 3.3 (controller-mode); only mode in Kafka 4.0.\n\nWhy it was needed:\n\n1. **Operational complexity**: running Kafka required also running ZooKeeper — a separate consensus system with its own JVM, tuning, monitoring, and failure modes. Two systems to debug instead of one.\n2. **Scalability**: ZooKeeper\'s znode model didn\'t scale well to clusters with millions of partitions. Metadata changes had to be serialized through a single ZooKeeper write path.\n3. **Faster controller failover**: in the ZooKeeper world, a controller failover required reading the entire cluster metadata from ZooKeeper — slow on large clusters. KRaft keeps a snapshot + log in Kafka itself; new controllers catch up via standard Raft semantics.\n4. **Recovery time**: Confluent reported controller failover going from minutes (ZooKeeper-backed, large clusters) to seconds (KRaft).\n\nTradeoff: clusters that have been running for years on ZooKeeper need a migration. Kafka shipped a "migration mode" that runs both side-by-side, but the bookkeeping is real ops work.',
    },
    {
      level: 'senior',
      question: 'Why does QUIC win on mobile and lose in some datacenter scenarios?',
      answer:
        'QUIC\'s mobile wins come from three properties:\n\n1. **Connection migration**: QUIC connections are identified by a connection ID, not by IP:port 4-tuple. When a phone moves from WiFi to cellular, the TCP connection would have died — QUIC keeps the same connection.\n2. **Per-stream loss recovery**: HTTP/2 over TCP suffers head-of-line blocking — a single lost TCP segment stalls every multiplexed stream. QUIC implements reliability per-stream, so a lost packet only stalls the one stream it carries.\n3. **0-RTT resumption**: returning users skip the handshake entirely (with replay-attack caveats for non-idempotent requests).\n\nWhere QUIC loses:\n\n1. **Datacenter east-west traffic**: no packet loss, no mobile migration. HTTP/2 over TCP is fine, and TCP\'s kernel offloads (e.g., GSO, hardware checksumming, kernel-bypass with DPDK) are more mature than QUIC\'s userland equivalents.\n2. **Pure throughput workloads**: a single large file download is one stream — per-stream loss recovery gives you nothing.\n3. **CPU overhead at the server**: HTTP/3 currently uses 2-3× more CPU per byte than HTTP/2 because TLS+congestion happen in userspace. Kernel offloads (e.g., NIC-level QUIC) are catching up but not universal.\n\nThe right deployment: HTTP/3 at the edge (toward users), HTTP/2 or gRPC over HTTP/2 between services in your own datacenter.',
    },
    {
      level: 'senior',
      question: 'How does chain replication achieve linearizable reads without quorum coordination?',
      answer:
        'In chain replication (van Renesse & Schneider, OSDI 2004), N replicas are arranged in an ordered chain: head → ... → tail.\n\n- **Writes** enter at the head, propagate down the chain. Each node applies the write and forwards to the next.\n- **Reads** are served by the tail only.\n- **Acks** flow back from the tail to the client.\n\nLinearizability falls out naturally: a write is only visible at the tail after it has propagated through every preceding node. Therefore, reads at the tail observe writes in the exact order they were committed at the head. No quorum read, no extra round-trip, no leases.\n\nWhat\'s traded:\n\n- **Write latency = chain length × hop latency.** A 5-node chain at 1ms per hop is 5ms minimum.\n- **Single tail bottleneck for reads** (in vanilla chain replication; CRAQ — Terrace & Freedman 2009 — extends it so reads can come from any node by tracking dirty/clean state per object).\n- **Configuration management is offloaded to a small Paxos/Raft service** — chain replication doesn\'t avoid consensus, it moves it to membership/leadership tracking.\n\nWhere it\'s used: Microsoft Azure Storage stream layer (Calder et al. SOSP 2011), Hibari, FAWN-KV, some HBase configurations.',
    },
    {
      level: 'staff',
      question: 'Why did CockroachDB pick Raft-per-shard over EPaxos?',
      answer:
        'EPaxos (Moraru, Andersen, Kaminsky, SOSP 2013) promises: no leader, any replica can commit a command locally in one round-trip for non-conflicting operations. In a geo-distributed deployment, that means any region pays only local-region latency for most writes — a huge win over leader-based protocols where every write pays WAN latency to one region.\n\nCockroachDB still chose **Raft per shard** (called "Range" in CockroachDB). The reasons:\n\n1. **Implementation maturity**: vetted Raft libraries (etcd-raft, hashicorp/raft, dragonboat, TiKV\'s raft) exist. EPaxos has no comparable production-grade library; building one is years of work and Jepsen-style bug exposure.\n2. **Predictable tail latency**: leader-based systems have one node that commits writes; tail latency is easier to reason about than EPaxos\'s dependency-tracking under conflict.\n3. **Many small consensus groups beat one big leaderless group**: CockroachDB has thousands of Ranges, each with its own Raft group, each with its own leader. A single global leader is never the bottleneck — different shards have different leaders, often in different regions, and writes to different shards proceed in parallel.\n4. **Conflict-heavy workloads punish EPaxos**: every contended write pays an extra round in EPaxos. Many real workloads have hot keys; the conflict path is more common than the textbook examples suggest.\n\nThe pattern is: **sharding is a better answer to the leader bottleneck than leaderless consensus** for most workloads.',
    },
    {
      level: 'staff',
      question: 'What\'s false sharing, and how does it limit "lock-free" scaling?',
      answer:
        '**False sharing** is the silent performance killer that hits "lock-free" data structures.\n\nThe mechanism: CPUs maintain cache coherence at **cache line granularity** (64 bytes on x86/ARM). When two cores write to *different* variables that happen to share a cache line, the MESI protocol forces the cache line to ping-pong between cores. Each write takes ~50-100ns (cross-core cache coherence) instead of ~1ns (local L1 write). All "lock-free" wins evaporate.\n\nClassic example: an array of per-thread counters. No locks, no contention in your code. But if the counters are adjacent in memory (`int counters[N_THREADS]`), they fit on the same cache line, and you\'ve just serialized through MESI — slower than a single shared atomic.\n\nFix: **pad each counter to a full cache line**. The patterns by language:\n- Java: `@Contended` (JDK 8+, behind `-XX:-RestrictContended`)\n- Go: explicit `[64]byte` padding, or wrap each in `struct{ x int64; _ [56]byte }`\n- Rust: `crossbeam::utils::CachePadded<T>`\n- C++: `std::hardware_destructive_interference_size` (C++17)\n\nWhere this matters most: high-throughput counters (metrics, scheduling, ring buffers — see Disruptor and LMAX). The performance delta between sharing and not sharing a cache line is often 10-50× under contention.\n\nDiagnostic: `perf c2c` on Linux is the cache-contention profiler. If your service hits CPU saturation at 60% on a multi-core box, false sharing or NUMA-remote allocation are the prime suspects.',
    },
    {
      level: 'staff',
      question: 'What does Percolator do that 2PC doesn\'t?',
      answer:
        'Percolator (Peng & Dabek, OSDI 2010) builds ACID transactions on top of Bigtable — a non-transactional wide-column store — without running a heavyweight 2PC protocol for each transaction.\n\nThe trick: **lock state lives in the data store itself**, in extra columns next to the data. There\'s no separate lock service, no separate transaction coordinator process.\n\nThe protocol:\n\n1. **Get a snapshot timestamp** from a globally-sorted timestamp oracle (a small, fast service).\n2. **Prewrite phase**: for each modified row, write the new value to a `data` column and a lock to a `lock` column. Designate one row as the **primary**; the others\' lock columns point to the primary\'s lock.\n3. **Commit phase**: atomically replace the primary\'s lock column with a `write` column pointing to the data. The primary\'s commit is the linearization point.\n4. **Cleanup**: secondaries lazily resolve their lock columns to write columns. If a transaction is interrupted, any node can check the primary\'s state and clean up.\n\nWhy this is better than vanilla 2PC for some workloads:\n\n- **No separate coordinator** — failure recovery uses Bigtable\'s own durability.\n- **No locks held in memory** — they\'re in the data store, survive crashes, and any node can resolve them.\n- **Snapshot isolation falls out** — readers use the timestamp oracle to filter visible writes.\n\nUsed by: Google\'s incremental web indexing (the original use case), TiDB / TiKV, FoundationDB has similar ideas.\n\nWhat it doesn\'t buy you: cross-shard latency is still high (multiple Bigtable round-trips per row). The "save coordinator overhead" win matters mostly at very high transaction rates with cheap individual operations.',
    },
  ],
};
