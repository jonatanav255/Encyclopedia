...
export const bank: QuestionBank = {
  ...
  questions: [
    ...
    {
      level: 'mid',
      question: 'What does a window function compute that GROUP BY can\'t?',
      answer:
        '`GROUP BY` collapses rows into one row per group; you lose the individual rows. A **window function** computes an aggregate or ranking across rows *without collapsing them* — each input row gets a computed value alongside.\n\nCanonical example: top-N per group. Without windows you write a correlated subquery or self-join; with windows it\'s a clean `ROW_NUMBER() OVER (PARTITION BY category ORDER BY score DESC)` then `WHERE rn <= 3` in an outer query.\n\nOther uses: running totals (`SUM(...) OVER (ORDER BY ... ROWS UNBOUNDED PRECEDING)`), moving averages (`AVG(...) OVER (... ROWS BETWEEN 6 PRECEDING AND CURRENT ROW)`), period-over-period change (`LAG(metric) OVER (ORDER BY date)`), and percentiles (`PERCENT_RANK`).\n\nGotchas: (1) Window functions evaluate **after** `WHERE` but **before** `LIMIT`, so filtering on the result needs a subquery. (2) The default frame when `ORDER BY` is present is `RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW`, which can include "peer" rows with equal sort keys — use `ROWS` for strict row-by-row semantics. (3) Performance hinges on indexes matching `PARTITION BY` + `ORDER BY` to skip the sort.',
    },
    {
      level: 'mid',
      question: 'When would you use a recursive CTE, and what safety patterns matter?',
      answer:
        'Recursive CTEs are how SQL expresses **iteration over a self-referencing relation**: organizational hierarchies, threaded comments, category trees, bill-of-materials computations, graph traversal. Anywhere "walk this self-join until you can\'t anymore."\n\nThe shape is `WITH RECURSIVE x AS (anchor UNION ALL recursive_query)`. `UNION ALL` (not `UNION`) is required — `UNION` deduplicates per iteration and is almost always a bug. Anchor returns the base case, recursive query references `x` and joins forward.\n\nThree safety patterns are non-negotiable:\n\n- **Cycle guard.** For graph-shaped data, accumulate a `path` array and check `NOT (next_node = ANY(path))` in the WHERE clause. Without it, cycles cause infinite iteration.\n- **Depth bound.** Add `array_length(path, 1) < N` as a hard limit. Catches both data bugs and runaway recursion in production.\n- **Statement timeout.** `SET LOCAL statement_timeout = \'30s\'` as a third backstop.\n\nPostgres 12+ inlines non-recursive CTEs by default (the old "CTE is an optimization fence" rule is gone). Recursive CTEs are always materialized. For deep graph work at scale, evaluate a graph database (Neo4j) or Postgres AGE — recursive CTEs are great for occasional hierarchy queries, not for billion-edge analytics.',
    },
    {
      level: 'staff',
