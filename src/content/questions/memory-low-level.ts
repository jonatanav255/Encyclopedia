...
export const bank: QuestionBank = {
  ...
  questions: [
    ...
    {
      level: 'senior',
      question: 'What is pointer compression, and when does it help?',
      answer:
        '**Pointer compression** stores object references as **32-bit offsets** within a known heap region instead of full 64-bit pointers. Decode is `base + offset` (one ADD); the 32 bits give you up to 4 GiB of heap addressable (HotSpot adds an 8-byte shift to extend to 32 GiB with `UseCompressedOops`).\n\nThe win is **cache density**: every reference is half the size, so a cache line holds twice as many. Pointer-chasing data structures (linked lists, trees, hash-table chains, B-trees) see double the fanout per cache line. On memory-bound workloads this is a real speed-up.\n\nThe cost is one extra ADD per dereference and, in JIT runtimes (V8, HotSpot), more complex codegen. CPUs hide the ADD via address-generation units, so the cache-line win dominates.\n\nThe pivot point: **above ~32 GiB heap**, the HotSpot JVM auto-disables `UseCompressedOops` because 32-bit offsets stop fitting. Bumping a JVM heap from 31 GiB to 33 GiB silently doubles every pointer\'s size, often increasing total RSS by 20-30%. V8 caps its "cage" at 4 GiB per isolate and lives within it.\n\nUse where: large pointer-rich heaps under 32 GiB, especially with pointer-chasing workloads. Don\'t use where: huge heaps, tight inner loops where one extra ADD per load measurably matters, or systems languages where you have other ways to control object layout.',
    },
    {
      level: 'staff',
      question: 'What is Meltdown and how does KPTI fix it, and why does my Postgres benchmark show extra context-switch overhead post-mitigation?',
      answer:
        '**Meltdown** (CVE-2017-5754) exploits the fact that out-of-order Intel CPUs speculatively complete a load to a kernel address from user space *before* the privilege check fires. Within the speculation window, the loaded byte is forwarded to a dependent load like `array[byte * 4096]`, which warms a cache line. The CPU rolls back the architectural state when the fault is raised, but the cache state stays. A FLUSH+RELOAD over the 256 candidate lines reveals the leaked byte. The attack is straight-line; no gadget training needed.\n\n**KPTI** (Kernel Page Table Isolation) fixes it by separating user-mode and kernel-mode page tables. While the process runs in user mode, the kernel\'s pages are simply **not mapped** — there\'s no virtual address for speculation to even target. A syscall switches to a second page table that has the kernel mapped, runs the syscall, then switches back.\n\nThe extra context-switch cost is the **CR3 write on every transition** (~50-200ns each), plus the **TLB flush** that comes with it (PCID helps reduce but doesn\'t eliminate). Postgres benchmarks see it because the workload is syscall-heavy: every `read`/`write`/`recv` traverses user↔kernel and pays the CR3 cost. Compute-heavy workloads that stay in user space barely notice. Realistic slowdowns: 3-8% on web servers, 5-15% on Redis/Postgres, up to 20% on syscall-saturated databases on older Intel.\n\nNote: Meltdown affected pre-Coffee-Lake Intel and IBM POWER. AMD, ARM (mostly), and Apple Silicon never needed KPTI. Newer Intel silicon (Ice Lake+) handles the issue in hardware; KPTI can be disabled there for a throughput win.',
    },
    {
      level: 'senior',
