...
export const bank: QuestionBank = {
  ...
  questions: [
    ...
    {
      level: 'junior',
      question: 'Why is `java.util.Stack` the wrong answer for a stack in modern Java?',
      answer:
        '`Stack` extends `Vector`, which is **synchronized**. Every `push`/`pop`/`peek` takes a lock you didn\'t ask for, on an object only one thread is touching. The Javadoc itself recommends using `Deque` instead: "A more complete and consistent set of LIFO stack operations is provided by the Deque interface and its implementations, which should be used in preference to this class."\n\nThe right answer in modern Java is `ArrayDeque`:\n\n```java\nDeque<Integer> stack = new ArrayDeque<>();\nstack.push(1);\nstack.push(2);\nstack.pop();   // 2\nstack.peek();  // 1\n```\n\nFaster than `Stack`, faster than `LinkedList`, no synchronization overhead. Same class also works as a queue (`offer`/`poll`) or full deque (`addFirst`/`pollLast`). One backing structure, three behaviors.\n\nThe one caveat: `ArrayDeque` rejects `null`. If you have legacy code that puts nulls into a `Stack`, it won\'t port directly — but storing nulls in a stack is almost always a smell anyway. For concurrent stacks, reach into `java.util.concurrent` — `ConcurrentLinkedDeque` for non-blocking, `LinkedBlockingDeque` for bounded.',
    },
    {
      level: 'mid',
      question: 'Three engineers reach for "Bloom filter," "HyperLogLog," and "Count-Min Sketch." They\'re solving different problems. Match each to its question.',
      answer:
        'Each answers a different question about a stream:\n\n- **Bloom filter** — "Is X in this set?" Answers "probably yes" or "definitely no." False positives, never false negatives. Used as a cheap pre-check before an expensive lookup (SSTable reads in Cassandra/RocksDB, CDN cache lookups, malicious-URL filters). ~10 bits per element at 1% false-positive rate.\n\n- **HyperLogLog** — "How many distinct elements?" Cardinality estimation. Stores nothing about individual elements, just maintains buckets of "max leading-zero hash" counts. ~12 KB per HLL gets you ~0.8% error on billions of distinct items. Redis `PFCOUNT`, Presto `approx_distinct`, ClickHouse `uniqHLL12` — all HLL.\n\n- **Count-Min Sketch** — "How often did element X appear?" Frequency estimation. A 2D array of counters indexed by k hashes; query returns the min across rows. Always over-estimates, never under. Pair with a min-heap to find top-k heavy hitters in bounded memory. Used in network telemetry, ad-fraud throttling, DDoS detection.\n\nThe shape is similar (hash, modular indexing, bounded memory, probabilistic answer) but the questions are different. Substituting one for another is a common interview trap. If you want **similarity** instead of any of these, that\'s **MinHash** — a fourth structure in the same family.\n\nAll of them merge across shards by combining their underlying arrays (OR for Bloom, max-per-bucket for HLL, sum for CMS). That property is what makes them work in distributed systems.',
    },
    {
      level: 'senior',
      question: 'Why does Redis use a skip list for sorted sets instead of a balanced BST?',
      answer:
        'Skip lists are a sorted linked list with random "express lanes" at higher levels. Expected O(log n) per operation — comparable to a balanced BST — but **no rotations**. That last property is the whole reason Redis picked one.\n\nThe direct comparison:\n\n- **Balanced BST (RB-tree, AVL)** — rotations on every insert and delete. Concurrent BSTs are famously hard; lock-free RB-tree implementations are research-grade. Java has no `ConcurrentTreeMap` for exactly this reason.\n- **Skip list** — inserts and deletes only rewire a constant number of forward pointers per level. Concurrent skip lists are tractable. `java.util.concurrent.ConcurrentSkipListMap` exists; lock-free reads, CAS writes.\n\nRedis ZSET specifically stores each member in **both** a skip list (sorted by score, for `ZRANGE`, `ZRANGEBYSCORE`, rank queries) and a hash map (keyed by member name, for O(1) `ZSCORE`). The skip list gives ordered access; the hash gives constant-time membership.\n\nRocksDB and LevelDB also use skip lists for the memtable — the same concurrency-without-rotation argument applies. Multiple writers append to the memtable without contending on a tree-wide rebalance.\n\nThe tradeoff: skip lists are RAM structures, not disk structures. They have ~2 forward pointers per node on average, and the random levels mean nodes vary in size. On disk, B+ trees beat them every time — locality wins. In RAM under contention, skip lists win.',
    },
    {
      level: 'senior',
      question: 'Explain the RUM conjecture in one paragraph, and use it to compare B+ trees against LSM trees.',
      answer:
        'The RUM conjecture (Athanassoulis et al. 2016) says you can\'t simultaneously minimize **R**ead amplification, **U**pdate amplification, and **M**emory amplification — every storage structure picks two to optimize and pays on the third. There\'s no free lunch.\n\nB+ trees minimize read amplification (1-2 page reads per lookup) and memory amplification (~1× the live data), at the cost of update amplification — every write is a random page write, and the WAL doubles every write. They win on read-heavy, locality-friendly workloads: classic OLTP, Postgres, MySQL/InnoDB, Oracle.\n\nLSM trees minimize update amplification (writes are sequential appends to an in-memory buffer that flushes to immutable files), at the cost of read amplification (a point lookup may scan multiple SSTables across multiple levels) and space amplification (1.1-2× the live data, depending on compaction strategy). They win on write-heavy workloads: Cassandra, RocksDB, LevelDB, ScyllaDB, HBase, CockroachDB (via Pebble), TiKV.\n\nLeveled compaction (LevelDB/RocksDB default) trades higher write amplification (10-30×) for lower read amplification. Tiered compaction (Cassandra default historically) trades lower write amp for higher read amp. Pick the strategy that matches your workload.\n\nThe practical lesson: when someone asks "why didn\'t you use a B+ tree here?" or "why not an LSM?", the answer is "I picked write-heavy or read-heavy, and accepted the cost on the other axis." Anyone trying to claim both is selling you something.',
    },
    {
      level: 'mid',
      question: 'You\'re indexing 50M geographic points for "find restaurants within 500m of me." Why does a B-tree on (lat, lng) lose, and what\'s the right structure?',
      answer:
        'A B-tree indexes a single total order — it can sort by lat OR by lng, but not by 2D proximity. Two points one meter apart in the real world can be at opposite ends of the index if you compose `(lat, lng)` as a single key. A radius query degenerates to "scan everything in the lat range, then filter by lng" — billions of comparisons.\n\nThe right structure is a **spatial index**, and you have three main flavors:\n\n- **R-tree (and R\\* tree)** — each node stores the minimum bounding rectangle (MBR) of its subtree. The R\\* variant has better split heuristics. PostGIS\\\' default GiST index, SQLite\\\'s `rtree` virtual table, JTS Topology Suite, GeoTools — all R-tree-family. Best for general geometries (points, polygons, lines).\n- **k-d tree** — recursive binary partition, alternating split dimensions. Great for static point sets and k-NN queries. Awkward updates.\n- **BKD tree** — block k-d tree, what Lucene and Elasticsearch use since version 6 (2016). A k-d tree adapted for paged storage.\n\nThe MBR query trick is what makes R-trees fast in databases: spatial queries run in two phases — **filter** (bounding-box overlap, fast and indexed) and **refine** (exact geometry check, slower, only on candidates). PostGIS\\\' `&&` operator is filter-only; `ST_Intersects` is filter-then-refine.\n\nAlternatives that get you 80% of the way without the tree: **geohash** strings encode 2D points into 1D keys where shared prefixes mean nearby points — store in any RDBMS B-tree. Redis GEO commands use this. **S2 cells** (Google) and **H3** (Uber) are more sophisticated hierarchical-cell systems with the same idea.\n\nFor radius queries on 50M points with frequent updates: PostGIS with GiST index, or MongoDB with a 2dsphere index, or a dedicated geo engine. Don\\\'t roll your own R-tree.',
    },
  ],
};
